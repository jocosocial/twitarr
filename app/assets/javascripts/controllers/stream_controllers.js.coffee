Twitarr.StreamViewController = Twitarr.Controller.extend Twitarr.SinglePhotoUploadMixin,
  ...
  actions:
    ...
    new: ->
      ...
      Twitarr.StreamPost.reply(@get('model.id'), @get('model.reply_text'), @get('photo_id'), @get('model.as_mod'), @get('model.as_admin')).fail((response) =>
...
Twitarr.StreamPageController = Twitarr.Controller.extend Twitarr.SinglePhotoUploadMixin,
  ...
  actions:
    ...
    new: ->
      ...
      return if @get('model.posting')
      @set 'model.posting', true
      Twitarr.StreamPost.new_post(@get('model.new_post'), @get('photo_id'), @get('model.as_mod'), @get('model.as_admin')).fail((response) =>
        @set 'model.posting', false
        if response.responseJSON?.error?
          @set 'errors', [response.responseJSON.error]
        else if response.responseJSON?.errors?
...
Twitarr.StreamMentionsPageController = Twitarr.Controller.extend Twitarr.SinglePhotoUploadMixin,
  actions:
    next_page: ->
      @transitionToRoute('stream.mentions_page', @get('model.next_page'))

Twitarr.StreamAuthorPageController = Twitarr.Controller.extend Twitarr.SinglePhotoUploadMixin,
  actions:
    next_page: ->
      @transitionToRoute('stream.author_page', @get('model.next_page'))
