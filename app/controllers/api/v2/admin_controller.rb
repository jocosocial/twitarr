  ...
  def update_user
    ...
    if params.key?(:role)
      ...
      if @current_user.role == User::Role::MODERATOR
        ...
        roleErrors.push('Only Admin and THO can ban or unban users.') if newRole == User::Role::BANNED || (@user.role == User::Role::BANNED && newRole != User::Role::BANNED)

        # Moderators cannot alter priviliged roles
        roleErrors.push('Only Admin and THO can change priviliged roles.') if @user.role != newRole && (@user.role >= User::Role::MODERATOR || newRole >= User::Role::MODERATOR)
      elsif @current_user.role < User::Role::ADMIN
        roleErrors.push('Only Admin can grant or revoke the admin role.') if @user.role != newRole && (@user.role >= User::Role::ADMIN || newRole >= User::Role::ADMIN)
      end
